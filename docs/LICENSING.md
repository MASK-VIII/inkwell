...
| Tier        | EPUB | Full exports / DOCX / archives | Cloud library sync | Backup size cap (compressed zip) |
|------------|------|--------------------------------|--------------------|----------------------------------|
| Free       | No   | No                             | No                 | —                                |
| Basic      | Yes  | No                             | Yes                | 2 GiB                            |
| Pro        | Yes  | Yes                            | Yes                | 20 GiB                           |

Enforcement: client checks zip size in `pushLibraryZip` / `forcePushLibraryZip` before upload (`src/lib/sync/syncEngine.ts`). Constants: `src/lib/cloudQuota.ts`. Storage bucket single-object limit is raised by migration `20260507180000_raise_libraries_bucket_limit.sql`.
...
2. **Run SQL migrations** so the database matches the repo:
   - **Option A — SQL Editor:** Open **SQL** → **New query**, paste the contents of `supabase/migrations/20260503120000_inkwell_library_sync.sql`, run it, then paste and run `20260505120000_user_entitlements.sql`, then `20260506120000_ebook_suite_cloud_sync.sql`, then `20260507180000_raise_libraries_bucket_limit.sql` (skip any file you already applied; the third file is safe to re-run and fixes Basic-tier cloud sync if you only ran the first entitlement migration). The bucket-limit migration raises the per-upload cap so 2 GiB / 20 GiB tier backups can upload.
